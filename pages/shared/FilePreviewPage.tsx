...
import { 
  ...
  ChevronRight, ZoomIn, ZoomOut, Plus, Save, Undo, Redo, ClipboardCheck
} from 'lucide-react';
...
export const FilePreviewPage: React.FC = () => {
  ...
  const [history, setHistory] = useState<DocumentAnnotations[]>([]);
  ...
  const handleSaveAll = async () => {
    ...
    try {
        ...
        showToast("Anotações técnicas salvas.", "success");
    } catch (e) {
        showToast("Falha ao salvar no Ledger.", "error");
    }
  };
    ...
    <div className="h-screen w-screen bg-[#020617] flex flex-col overflow-hidden font-sans text-slate-200">
      <header className="h-16 flex items-center justify-between px-6 bg-[#081437]/90 backdrop-blur-xl border-b border-white/5 z-20 shrink-0">
        <div className="flex items-center gap-4">
          ...
          <div className="min-w-0">
            <h2 className="text-white text-xs font-black uppercase tracking-[3px] truncate max-w-md">
              {currentFile?.name || "Acessando Ativo..."}
            </h2>
          </div>
        </div>
        ...
        <div className="flex items-center gap-4">
            <button 
                ...
                className="px-6 py-2 bg-blue-600 hover:bg-blue-500 text-white rounded-xl text-[10px] font-black uppercase tracking-[2px] flex items-center gap-2 transition-all disabled:opacity-30"
            >
                {isSyncing ? <Loader2 size={14} className="animate-spin" /> : <Save size={14} />}
                Salvar Alterações
            </button>
            <button onClick={handleDownload} disabled={!url} className="p-2.5 bg-white/5 hover:bg-white/10 text-slate-400 rounded-xl border border-white/5 disabled:opacity-20">
              <Download size={18} />
            </button>
        </div>
      </header>
      ...
      <div className="flex-1 relative overflow-hidden bg-[#020617]">
        {url ? (
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center text-slate-600 gap-4">
             ...
             <p className="text-[10px] font-black uppercase tracking-[4px]">Sincronizando Viewport...</p>
          </div>
        )}
      </div>
      ...
      <div className="fixed bottom-8 left-1/2 -translate-x-1/2 z-[100] flex items-center gap-3 bg-[#081437]/95 backdrop-blur-3xl border border-white/10 p-2.5 rounded-[2.5rem] shadow-[0_40px_80px_rgba(0,0,0,0.8)] animate-in slide-in-from-bottom-10">
        <div className="flex items-center gap-2 bg-white/5 p-1 rounded-full border border-white/5 px-4">
          <button disabled={pageNum <= 1} onClick={() => setPageNum(pageNum - 1)} className="p-2 text-slate-400 hover:text-white disabled:opacity-20"><ChevronLeft size={18} /></button>
          <span className="text-[10px] font-black text-blue-400 min-w-[50px] text-center tracking-[2px]">{pageNum} / {numPages || '--'}</span>
          <button disabled={pageNum >= numPages} onClick={() => setPageNum(pageNum + 1)} className="p-2 text-slate-400 hover:text-white disabled:opacity-20"><ChevronRight size={18} /></button>
        </div>

        <div className="flex items-center gap-1.5 px-1 border-r border-white/10">
          <ToolButton icon={Hand} active={activeTool === 'hand'} onClick={() => { setActiveTool('hand'); setShowToolsMenu(false); }} label="Mão" />
          
          <div className="relative">
            <button 
              onClick={() => setShowToolsMenu(!showToolsMenu)}
              className={`p-3 rounded-xl transition-all flex flex-col items-center gap-0.5 ${showToolsMenu ? 'bg-white text-slate-900' : 'bg-white/5 text-slate-400 hover:bg-white/10'}`}
            >
        ...
        {mode === 'audit' ? (
           <button onClick={handleReturn} className="ml-2 px-8 py-3 bg-[#b23c0e] hover:bg-orange-600 text-white rounded-full font-black text-[10px] uppercase tracking-[3px] shadow-xl active:scale-95 flex items-center gap-3">
              <PlayCircle size={18} /> RETORNAR AO FLUXO
           </button>
        ) : (
           canAudit && (
            <button onClick={() => navigate(`/quality/inspection/${fileId}`)} className="ml-2 px-8 py-3 bg-[#b23c0e] hover:bg-orange-600 text-white rounded-full font-black text-[10px] uppercase tracking-[3px] shadow-xl active:scale-95 flex items-center gap-3">
                <ClipboardCheck size={18} /> INICIAR AUDITORIA
            </button>
           )
        )}
      </div>
...
const ToolButton = ({ icon: Icon, active, onClick, label, disabled }: any) => (
  <button 
    ...
    onClick={onClick}
    title={label}
    className={`p-3 rounded-xl transition-all relative group flex items-center justify-center disabled:opacity-20 ${active ? 'bg-blue-600 text-white shadow-lg' : 'text-slate-400 hover:text-white hover:bg-white/10'}`}
  >
